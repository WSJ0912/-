...
import { Activity, ArrowUpRight, ClipboardCheck, FileBarChart2, FileImage, FileText, FlaskConical, FolderOpen, KeyRound, LogIn, LockKeyhole, RefreshCw, ScanLine, Settings2, ShieldCheck, Sparkles, Upload, UserPlus, X } from "lucide-react";
...
import { formatProbability, labels, NavKey, request, Role } from "./api";

type BootState = { connected: boolean; requiresSetup: boolean; model: any; message?: string };
type Study = { studyId: string; createdAt: string; modality: string; bodyPart: string; viewPosition: string; status: string };
type Prediction = { predictionId: string; studyId: string; modelId: string; modelVersion: string; probabilities: Record<string, number | null>; thresholds: Record<string, number>; camAvailable: boolean };
type ReviewMap = Record<string, "confirmed" | "denied" | "uncertain">;

const navItems: Array<{ id: NavKey; label: string; icon: typeof ScanLine; hint: string }> = [
  { id: "queue", label: "检查队列", icon: FolderOpen, hint: "导入与准入" },
  { id: "workbench", label: "阅片工作区", icon: ScanLine, hint: "AI 结果与复核" },
  { id: "reports", label: "报告中心", icon: FileText, hint: "版本与导出" },
  { id: "experiments", label: "实验结果", icon: FlaskConical, hint: "聚合指标" },
  { id: "models", label: "模型管理", icon: Activity, hint: "清单与校验" },
  { id: "admin", label: "管理员设置", icon: Settings2, hint: "账号与助手" },
];
...
function App() {
  ...
  if (!role) return <AuthScreen requiresSetup={boot.requiresSetup} onAuthenticated={(nextRole, nextUser) => { setRole(nextRole); setUserId(nextUser); setBoot((current) => ({ ...current, requiresSetup: false })); }} onError={fail} />;
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <nav aria-label="主导航">{navItems.map(({ id, label, icon: Icon, hint }) => <button key={id} className={`nav-item ${nav === id ? "active" : ""} ${id === "admin" && role !== "admin" ? "muted" : ""}`} onClick={() => { if (id !== "admin" || role === "admin") setNav(id); }} disabled={id === "admin" && role !== "admin"}><Icon size={18} /><span><b>{label}</b><small>{hint}</small></span>{id === "queue" && <i className="nav-count">0</i>}</button>)}</nav>
      <div className="sidebar-bottom"><div className="privacy-note"><ShieldCheck size={16} /><span>本地优先<br /><small>影像不会离开此设备</small></span></div><div className="user-chip"><div className="avatar">{role === "admin" ? "A" : "D"}</div><div><b>{role === "admin" ? "管理员" : "医生"}</b><small>{userId || "当前会话"}</small></div><IconButton icon={LockKeyhole} label="结束会话" onClick={() => { void window.medicalApi.logout(); setRole(null); setUserId(""); }} /></div></div>
    </aside>
    <main className="main-area">
      ...
      <div className="content-area">{nav === "queue" && <QueueView onError={fail} onNotice={flash} onOpenWorkbench={() => setNav("workbench")} />}{nav === "workbench" && <WorkbenchView model={boot.model} onError={fail} onNotice={flash} />}{nav === "reports" && <ReportsView onError={fail} onNotice={flash} />}{nav === "experiments" && <ExperimentsView onError={fail} onNotice={flash} />}{nav === "models" && <ModelsView model={boot.model} role={role} onError={fail} onNotice={flash} onInstalled={() => setRefresh((value) => value + 1)} />}{nav === "admin" && <AdminView onError={fail} onNotice={flash} />}</div>
    </main>
  </div>;
}
...
function AuthScreen({ requiresSetup, onAuthenticated, onError }: { requiresSetup: boolean; onAuthenticated: (role: Role, userId: string) => void; onError: (reason: unknown) => void }) {
  ...
  async function submit(event: React.FormEvent) { event.preventDefault(); setBusy(true); try { const result = await request(setupMode ? "/api/setup" : "/api/login", "POST", { username, password }); onAuthenticated(result.role, result.userId); } catch (reason) { onError(reason); } finally { setBusy(false); } }
  return <div className="center-screen"><div className="auth-panel"><div className="auth-header"><div className="brand-mark"><ScanLine size={19} /></div><div><p className="eyebrow">MEDICAL IMAGING PLATFORM</p><h1>{setupMode ? "创建首个管理员" : "进入本地工作区"}</h1></div></div><p className="muted">帐号与数据库保存在本机。密码使用 Argon2id 处理，管理员不能代替医生确认报告。</p><form onSubmit={submit} className="form-stack"><label>用户名<input value={username} onChange={(event) => setUsername(event.target.value)} autoComplete="username" required minLength={3} /></label><label>密码<input type="password" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete={setupMode ? "new-password" : "current-password"} required minLength={12} /></label><button className="primary-button wide" disabled={busy}>{setupMode ? <UserPlus size={17} /> : <LogIn size={17} />}{busy ? "处理中…" : setupMode ? "创建管理员并继续" : "登录"}</button></form>{!requiresSetup && <button className="text-button" onClick={() => setSetupMode((value) => !value)}>{setupMode ? "返回登录" : "首次使用？创建管理员"}</button>}<div className="auth-foot"><ShieldCheck size={15} />离线核心功能可用 · 无模型时不会生成结果</div></div></div>;
}
...
function QueueView({ onError, onNotice, onOpenWorkbench }: { onError: (reason: unknown) => void; onNotice: (message: string) => void; onOpenWorkbench: () => void }) {
  ...
  const reload = async () => { try { setStudies(await request("/api/studies")); } catch (reason) { onError(reason); } };
...
function WorkbenchView({ model, onError, onNotice }: { model: any; onError: (reason: unknown) => void; onNotice: (message: string) => void }) {
  const [studies, setStudies] = useState<Study[]>([]); const [selected, setSelected] = useState<Study | null>(null); const [prediction, setPrediction] = useState<Prediction | null>(null); const [review, setReview] = useState<ReviewMap>({}); const [notes, setNotes] = useState(""); const [busy, setBusy] = useState(false);
  useEffect(() => { request("/api/studies").then(setStudies).catch(onError); }, []);
  async function runPrediction() { if (!selected) return; try { setBusy(true); setPrediction(await request(`/api/studies/${selected.studyId}/predict`, "POST")); onNotice("推理完成，原始概率已锁定"); } catch (reason) { onError(reason); } finally { setBusy(false); } }
  async function saveReview() { if (!selected || !prediction) return; try { setBusy(true); await request("/api/reviews", "POST", { studyId: selected.studyId, predictionId: prediction.predictionId, decisions: review, notes }); onNotice("复核已保存"); } catch (reason) { onError(reason); } finally { setBusy(false); } }
  return <div className="view-stack"><SectionHeader eyebrow="REVIEW / MULTI-LABEL OUTPUT" title="阅片工作区" description="AI 原始概率不可修改；医生复核单独记录为确认、否认或不确定。" action={<div className="workbench-actions"><select value={selected?.studyId || ""} onChange={(event) => { const value = studies.find((item) => item.studyId === event.target.value) || null; setSelected(value); setPrediction(null); setReview({}); }}><option value="">选择检查</option>{studies.map((study) => <option key={study.studyId} value={study.studyId}>{study.studyId} · {study.viewPosition}</option>)}</select><button className="primary-button" onClick={() => void runPrediction()} disabled={!selected || !model?.installed || busy}><Activity size={17} />{busy ? "推理中…" : "运行推理"}</button></div>} /><div className="workbench-grid"><DicomViewport studyId={selected?.studyId} /><aside className="result-panel"><div className="result-header"><div><p className="eyebrow">AI SCREENING</p><h3>{prediction ? `${prediction.modelId} / ${prediction.modelVersion}` : "尚未运行"}</h3></div>{prediction ? <span className="tag good">ONNX 原始结果</span> : <span className="tag neutral">--</span>}</div>{!model?.installed && <div className="inline-warning"><Activity size={16} /><span>尚未安装模型。请先导入已验证的 `.medmodel`，系统不会生成模拟结果。</span></div>}{prediction ? <div className="observation-list">{labels.map(([key, zh]) => <div className="observation" key={key}><div className="observation-label"><span>{zh}</span><small>{key}</small></div><strong>{formatProbability(prediction.probabilities[key])}</strong><div className="probability-track"><i style={{ width: `${Math.max(0, Math.min(100, Number(prediction.probabilities[key] || 0) * 100))}%` }} /></div><div className="review-controls">{(["confirmed", "denied", "uncertain"] as const).map((choice) => <button className={review[key] === choice ? "selected" : ""} key={choice} onClick={() => setReview((current) => ({ ...current, [key]: choice }))}>{choice === "confirmed" ? "确认" : choice === "denied" ? "否认" : "不确定"}</button>)}</div></div>)}</div> : <div className="result-empty"><ClipboardCheck size={22} /><p>选择检查并运行已安装模型</p><small>所有未知概率显示为 --</small></div>}{prediction && <div className="review-footer"><textarea value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="复核备注（可选）" /><button className="quiet-button" onClick={() => void saveReview()} disabled={busy}><ClipboardCheck size={16} />保存复核</button></div>}</aside></div></div>;
}

function ReportsView({ onError, onNotice }: { onError: (reason: unknown) => void; onNotice: (message: string) => void }) {
  const [studyId, setStudyId] = useState(""); const [body, setBody] = useState("检查所见：\n\n印象：\n"); const [report, setReport] = useState<any>(null); const [assistantCautions, setAssistantCautions] = useState<string[]>([]); const [busy, setBusy] = useState(false);
  async function save() { if (!studyId) return onError(new Error("请先填写检查编号")); try { setBusy(true); const next = await request("/api/reports/draft", "POST", { studyId, body, reportId: report?.reportId }); setReport(next); onNotice(`已保存第 ${next.revision} 版草稿`); } catch (reason) { onError(reason); } finally { setBusy(false); } }
  async function confirm() { if (!report) return; try { setBusy(true); setReport(await request("/api/reports/confirm", "POST", { reportId: report.reportId, revision: report.revision })); onNotice("报告已锁定"); } catch (reason) { onError(reason); } finally { setBusy(false); } }
  async function exportPdf() {
    if (!report || report.status !== "confirmed") return;
    try {
      setBusy(true);
      const result = await window.medicalApi.exportReport(report.reportId, report.revision, `${studyId}-阅片报告-v${report.revision}.pdf`);
      if (result.exported) onNotice("PDF 已导出到所选位置");
    } catch (reason) { onError(reason); } finally { setBusy(false); }
  }
  async function organizeWithAssistant() { try { setBusy(true); const result = await request("/api/assistant/report", "POST", { observations: [], review: {}, clinicianText: body }); setBody(result.draft); setAssistantCautions(result.cautions || []); onNotice("助手草稿已返回，仍需医生编辑确认"); } catch (reason) { onError(reason); } finally { setBusy(false); } }
  return <div className="view-stack"><SectionHeader eyebrow="REPORTING / VERSION CONTROL" title="报告中心" description="报告正文可版本化编辑；医生确认后锁定，并可导出正文与 AI 附录。" action={<span className="tag neutral">医生确认必需</span>} /><div className="report-layout"><section className="panel report-editor"><div className="panel-title"><div><h3>阅片报告正文</h3><p>不自动填入未经医生确认的结论。</p></div>{report && <span className={`tag ${report.status === "confirmed" ? "good" : "warn"}`}>{report.status === "confirmed" ? "已确认" : `草稿 v${report.revision}`}</span>}</div><label>检查编号<input value={studyId} onChange={(event) => setStudyId(event.target.value)} placeholder="例如 ST-..." disabled={report?.status === "confirmed"} /></label><textarea className="report-textarea" value={body} onChange={(event) => setBody(event.target.value)} disabled={report?.status === "confirmed"} />{assistantCautions.length > 0 && <div className="assistant-cautions">{assistantCautions.map((item) => <span key={item}>{item}</span>)}</div>}<div className="editor-actions"><button className="quiet-button" onClick={() => void organizeWithAssistant()} disabled={busy || report?.status === "confirmed"}><Sparkles size={16} />助手整理</button><button className="quiet-button" onClick={() => void save()} disabled={busy || report?.status === "confirmed"}><FileText size={16} />保存草稿</button>{report?.status === "confirmed" ? <button className="primary-button" onClick={() => void exportPdf()} disabled={busy}><FileText size={16} />导出 PDF</button> : <button className="primary-button" onClick={() => void confirm()} disabled={!report || busy}><LockKeyhole size={16} />确认并锁定</button>}</div></section><aside className="panel report-aside"><div className="panel-title"><div><h3>版本记录</h3><p>AI 附录包含原始概率、阈值和模型哈希。</p></div><FileBarChart2 size={19} /></div>{report ? <div className="revision-item"><span className="revision-number">v{report.revision}</span><div><strong>{report.status === "confirmed" ? "医生已确认" : "当前草稿"}</strong><small>{new Date(report.createdAt).toLocaleString("zh-CN")}</small></div></div> : <div className="table-empty">保存后显示版本</div>}<div className="pdf-note"><FileText size={16} /><span>{report?.status === "confirmed" ? "报告已锁定，可导出 PDF" : "PDF 导出在报告确认后开放"}<br /><small>正文与 AI 溯源附录一并生成</small></span></div></aside></div></div>;
}

function ExperimentsView({ onError, onNotice }: { onError: (reason: unknown) => void; onNotice: (message: string) => void }) { const [bundle, setBundle] = useState<any>(null); const [assistant, setAssistant] = useState<any>(null); const [busy, setBusy] = useState(false); async function select() { try { setBusy(true); const imported = await window.medicalApi.importExperiment(); if (!imported) return; setBundle(imported); setAssistant(null); onNotice("实验包已导入；仅展示聚合指标"); } catch (reason) { onError(reason); } finally { setBusy(false); } } async function summarize() { if (!bundle) return; try { setBusy(true); setAssistant(await request("/api/assistant/experiment", "POST", { aggregateMetrics: bundle.aggregateMetrics || {}, perClassMetrics: bundle.perClassMetrics || {}, experimentNotes: "" })); onNotice("实验摘要已生成，结论仍需研究者核对"); } catch (reason) { onError(reason); } finally { setBusy(false); } } return <div className="view-stack"><SectionHeader eyebrow="RESEARCH / AGGREGATE ONLY" title="实验结果" description="`.medexperiment` 不含影像、MIMIC 标识或患者级预测；未知指标保持 --。" action={<div className="experiment-actions">{bundle && <button className="quiet-button" onClick={() => void summarize()} disabled={busy}><Sparkles size={16} />助手摘要</button>}<button className="primary-button" onClick={select} disabled={busy}><FolderOpen size={17} />导入实验包</button></div>} />{bundle ? <><div className="metric-grid">{Object.entries(bundle.aggregateMetrics || {}).map(([key, value]) => <div className="metric-card" key={key}><span>{key}</span><strong>{typeof value === "number" ? value.toFixed(4) : "--"}</strong><small>聚合结果</small></div>)}</div>{assistant && <section className="panel assistant-summary"><div className="panel-title"><div><h3>实验助手摘要</h3><p>仅基于聚合指标，不含患者级数据。</p></div><Sparkles size={18} /></div><p>{assistant.summary}</p>{assistant.limitations?.length > 0 && <ul>{assistant.limitations.map((item: string) => <li key={item}>{item}</li>)}</ul>}</section>}</> : <div className="empty-panel tall"><div className="empty-icon"><FlaskConical size={21} /></div><h3>尚无实验包</h3><p>导入后可查看聚合指标、每类指标、曲线和模型卡。</p></div>}</div>; }

function ModelsView({ model, role, onError, onNotice, onInstalled }: { model: any; role: Role; onError: (reason: unknown) => void; onNotice: (message: string) => void; onInstalled: () => void }) { const [selection, setSelection] = useState<{ selectionId: string; name: string } | null>(null); async function select() { try { setSelection(await window.medicalApi.selectModelPackage()); } catch (reason) { onError(reason); } } async function install() { if (!selection) return; try { await window.medicalApi.installSelectedModel(selection.selectionId); setSelection(null); onInstalled(); onNotice("模型包已通过清单与 SHA-256 校验并激活"); } catch (reason) { onError(reason); } } return <div className="view-stack"><SectionHeader eyebrow="MODEL GOVERNANCE" title="模型管理" description="只接受包含完整 14 项输出、预处理定义、阈值、许可和文件哈希的 `.medmodel`。" action={role === "admin" && <button className="primary-button" onClick={select}><FolderOpen size={17} />选择模型包</button>} /><div className="model-hero"><div className="model-symbol"><Activity size={25} /></div><div className="model-copy"><p className="eyebrow">ACTIVE DEPLOYMENT</p><h2>{model?.installed ? model.modelId : "尚未安装模型"}</h2><p>{model?.installed ? `版本 ${model.version} · ${model.ready ? "CPU 推理就绪" : "运行时不可用"}` : "无模型时禁止生成任何预测；请由管理员手动导入许可明确的 .medmodel。"}</p>{selection && <div className="selected-file"><FileText size={15} />{selection.name}<button onClick={() => setSelection(null)} aria-label="清除选择"><X size={15} /></button></div>}{selection && role === "admin" && <button className="quiet-button" onClick={install}>验证并激活</button>}</div><div className={`model-state ${model?.installed ? "ready" : "missing"}`}><span className="status-dot" />{model?.installed ? "已安装" : "缺少模型"}</div></div><div className="panel"><div className="panel-title"><div><h3>部署约束</h3><p>不会自动下载未确认许可的权重。</p></div><ShieldCheck size={19} /></div><div className="constraint-grid"><div><b>输入</b><span>320 × 320 / 单通道</span></div><div><b>输出</b><span>CheXpert 14 项 logits</span></div><div><b>数据范围</b><span>成人正位 AP / PA</span></div><div><b>解释</b><span>特征图 / CAM（如可用）</span></div></div></div></div>; }
