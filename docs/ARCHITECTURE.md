...
`去标识化影像 -> 固定预处理 -> ONNX logits + 特征图 + 逐类 CAM -> 14 项概率 -> 独立医生复核`

只有显式的 `[N,14,H,W]` `cams` 输出会被标记、持久化并显示为 CAM。旧模型的通用特征图不会被当作可解释热力图。
...
v0.1 **没有实现模型自动下载**。仓库中不存在任何拉取或校验远端权重的代码路径；
唯一的分发方式是管理员在本机手动导入 `.medmodel`，导入时校验固定 14 项标签、
全部文件哈希、ZIP 路径安全和许可证字段非空。

只有在最终权重分发许可确认后才会考虑新增自动下载；届时必须同时实现版本、来源、
许可证和 SHA-256 校验，并默认关闭。在许可确认之前，任何关于“自动下载模型”的
说法都属于**未实现**，而不是“已实现但未启用”。
